#!/usr/bin/env python3
# backup-monitor — Ed25519 license key generator (developer-only)
...
def generate_keypair():
    ...
    key_path = os.path.expanduser("~/.backup-monitor-signing-key.pem")
    ...
    print()
    print("Embed this in backup_monitor/compliance.py:")
...
def sign_fingerprint(fingerprint: str):
    ...
    license_key = f"bkc-{encoded}"

    print(f"Fingerprint: {fingerprint}")
    print(f"License key: {license_key}")
    print()
    print("To activate:")
    print(f"  backup-monitor --license-key {license_key}")
    print("  # or")
    print(f"  export BACKUP_MONITOR_LICENSE_KEY={license_key}")


def main():
    parser = argparse.ArgumentParser(
        description="Generate Ed25519 license keys for backup-monitor",
    )
    parser.add_argument(
        "--generate-keypair",
        action="store_true",
    ...
    parser.add_argument(
        "fingerprint",
        nargs="?",
        help="Cluster fingerprint to sign (from backup-monitor --show-fingerprint)",
    )
